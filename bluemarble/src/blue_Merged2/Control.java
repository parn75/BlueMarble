...
public class Control {
	...
	public final void GAME_CONTROL(int controlNum) {
		...
		case 1:
			...
			dicetot = firstDics+secondDics;
			}
			System.out.println("주사위 1 : " + firstDics + 
							   "\n주사위 2 : " + secondDics+ 
							   "\n주사위 합 : " + playerData.get(nickName[turnCP]).getUserDics());
			playerData.get(nickName[turnCP]).setPlayerCerruntPosition(playerData.get(nickName[turnCP]).getPlayerCerruntPosition() + dicetot);
	...
	public void recieveData(String from, Object data) {
		...
		for(int i=0; i<nickName.length;i++){
			...
			if(nickName[i].equals(from)){
				...
				this.dicetot=gd.dicetot;
				System.out.println("주사윅밧"+gd.dicetot);
				playerData.get(nickName[turnCP]).setUserDics(this.dicetot);				System.out.println("werwer"+event);
