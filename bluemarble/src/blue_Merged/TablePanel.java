...
public class TablePanel extends JPanel implements MouseListener, Runnable {
	...
	protected final int[][] profileXY = { { 20, 20 }, { 1000, 20 },
			{ 20, 600 }, { 1000, 600 } }; 
	...
	protected void paintComponent(Graphics g) {
		...
		for(int i=0; i<control.getNickName().length;i++){
			System.out.println("렝스"+control.getNickName().length);
			if(!control.getNickName()[i].equals("None"))
		g2.drawImage(
				drawRoom(control.getNickName()[i],
						control.playerData.get(control.getNickName()[i])
								.getMoney(), 255-50*i,255-50*i),profileXY[i][0],profileXY[i][1], this);
		}
		// 좌표위치 테스트
		...
		for (Building t : buildingList) {
			...
			for (int j = 1; j < 4; j++) {
				if (buildingXY[t.thisXY * 3 + (j-1) ][0] != 0
						...
						&&control.cityMap.get(
								control.getCityNameList()[t.thisXY]).
								getBuildList()[j]==true)
